...
class ProductsTracker(Thread):
    ...
    def run(self):
        ...
        self.shops = Shops.objects.filter(id__in=ids_list).order_by('-id')
    ...
    def load_products(self) -> Union[list[Union[QuerySet, Shops]], None]:
        for shop in self.shops:
            try:
                ...
                response = requests.get(self.product_info(shop.shop_url), proxies=proxy).json()
                ...
                self.print_style(f"Added {shop.shop_url.replace('https://', '')} "
                                 f"with {self.products[shop.shop_url]['products'].__len__()} "
                                 f"products to memory database.")
            except (RequestException, FreeProxyException) as e:
                self.print_style(f"Link {shop.shop_url.replace('https://', '')} returned {e}")
        return self.shops
    ...
    def on_new_sale(self, shop_url: str, data: dict, sold_variant: [dict, None]):
        ...
        # /** ** self.print_styles the result that is sent to discord ** **/
    ...
    def check_for_sales(self, shop_url: str):
        def set_interval(func, sec, shop_url_):
            def func_wrapper():
                set_interval(func, sec, shop_url_)
                func(shop_url_)
        ...
        set_interval(self.check_for_sales, interval, shop_url)
        proxy_str: str = FreeProxy(rand=True).get()
        schema, ip = proxy_str.split('://')
        proxy = {
            schema: ip
        }
        data: dict = requests.get(self.product_info(shop_url), proxies=proxy).json()
    ...
    def track():
        ...
        while True:
            try:
                ...
                break
        shops = Shops.objects.filter(has_products=True, track_enabled=True).order_by('-id')
